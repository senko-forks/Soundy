...
namespace Soundy
{
    ...
    {
        private const int CurrentConfigVersion = 4;   // ↔ Plugin-Version 1.2
        public int Version { get; set; } = CurrentConfigVersion;

        public string PenumbraPath { get; set; } = "";
        public float Volume { get; set; } = 1.5f;
        ...
        public AudioPreset EqPreset { get; set; } = AudioPreset.Neutral;
        ...
        {
            if (!Enum.IsDefined(typeof(AudioPreset), EqPreset))
                EqPreset = AudioPreset.Neutral;
        }
    }
}
