...
namespace Soundy.Windows
{
    public class MainWindow : Window, IDisposable
    {
        ...
        public override void Draw()
        {
            ...
            {
                var dirPath = Path.Combine(plugin.Configuration.PenumbraPath, selectedMod);
                // Scannt alle .json, extrahiert die PAP-Referenzen und sucht in den PAPs nach SCDs.
                try
                {
                    papEntries = PapManager.ScanForPapDetailsGrouped(dirPath, ref processState);
                }
                catch (Exception ex)
                {
                ...
                // Optional: Auto-select, wenn nur ein PAP gefunden wurde.
                if (papEntries.Count == 1)
                {
                    papEntries[0].Selected = true;
                    userSelectedPapEntries = new List<GroupedPapEntry> { papEntries[0] };
                }
                papListLoaded = true;
            }
            ...
            else
            {
                ...
                if (ImGui.BeginTable("papTable", 4, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY, new Vector2(0,200)))
                {
                    ...
                    foreach (var entry in visible)
                    {
                        ...
                        ImGui.TextUnformatted(entry.PapPath);

                        // Spalte 3: Anzahl der SCD-Details in dieser PAP
                        ...
                        // Spalte 4: Details-Button (öffnet Popup)
        ...
        private async Task DownloadConvertAsync()
        {
            try
            {
                ...
                try
                {
                    FileManager.AddSong(dirPath, finalSCDPaths, scdPath, saveName);
                }
                catch (Exception ex)
                {
